var htmx = (function() {
  ...
  function oobSwap(e, t, n, r) {
    ...
    return s.length ? (forEach(
      ...
      function(a) {
        ...
        triggerEvent(a, "htmx:oobBeforeSwap", h) && (a = h.target, h.shouldSwap && (handlePreservedElements(l), swapWithStyle(i, a, a, l, n), restorePreservedElements()), forEach(n.elts, function(c) {
  ...
  function handleAttributes(e, t, n) {
    forEach(t.querySelectorAll("[id]"), function(r) {
      ...
      if (o && o.length > 0) {
        const i = o.replace("'", "\\'"), s = r.tagName.replace(":", "\\:"), a = asParentNode(e), l = a && a.querySelector(s + "[id='" + i + "']");
        if (l && l !== a) {
          const u = r.cloneNode();
          cloneAttributes(r, l), n.tasks.push(function() {
            cloneAttributes(r, u);
          });
        }
      }
  ...
  function swap(e, t, n, r) {
      ...
      if (n.swapStyle === "textContent")
        ...
        if (f.title = r.title || d.title, r.historyRequest && (d = d.querySelector("[hx-history-elt],[data-hx-history-elt]") || d), r.selectOOB) {
          ...
          for (let m = 0; m < E.length; m++) {
            ...
            const x = w[1] || "true", p = d.querySelector("#" + C);
            p && oobSwap(x, p, f, u);
          }
        }
    ...
    if (a && triggerEvent(l, "htmx:beforeTransition", r.eventInfo) && typeof Promise < "u" && // @ts-ignore experimental feature atm
  ...
  function parseAndCacheTrigger(e, t, n) {
    ...
    do {
      ...
      const a = o.length, l = consumeUntil(o, /[,\[\s]/);
      if (l !== "")
        if (l === "every") {
      ...
      o.length === a && triggerErrorEvent(e, "htmx:syntax:error", { token: o.shift() }), consumeUntil(o, NOT_WHITESPACE);
    } while (o[0] === "," && o.shift());
    return n && (n[t] = r), r;
  }
  ...
  function boostElement(e, t, n) {
    if (e instanceof HTMLAnchorElement && isLocalLink(e) && (e.target === "" || e.target === "_self") || e.tagName === "FORM" && String(getRawAttribute(e, "method")).toLowerCase() !== "dialog") {
      ...
      n.forEach(function(i) {
        addEventListener(e, function(s, a) {
          const l = asElement(s);
          if (eltIsDisabled(l)) {
            cleanUpElement(l);
            return;
  ...
  function addEventListener(e, t, n, r, o) {
    ...
    let s;
    r.from ? s = querySelectorAllExt(e, r.from) : s = [e], r.changed && ("lastValue" in i || (i.lastValue = /* @__PURE__ */ new WeakMap()), s.forEach(function(a) {
      i.lastValue.has(r) || i.lastValue.set(r, /* @__PURE__ */ new WeakMap()), i.lastValue.get(r).set(a, a.value);
      ...
      const l = function(u) {
        ...
        if (ignoreBoostedAnchorCtrlClick(e, u) || ((o || shouldCancel(u, a)) && u.preventDefault(), maybeFilterEvent(r, e, u)))
          return;
  ...
  function getInputValues(e, t) {
    ...
    if (s.lastButtonClicked && (a = a && s.lastButtonClicked.formNoValidate !== !0), t !== "get" && processInputValue(n, o, i, getRelatedForm(e), a), processInputValue(n, r, i, e, a), s.lastButtonClicked || e.tagName === "BUTTON" || e.tagName === "INPUT" && getRawAttribute(e, "type") === "submit") {
    ...
    return forEach(l, function(u) {
      processInputValue(n, r, i, asElement(u), a), matches(u, "form") || forEach(asParentNode(u).querySelectorAll(INPUT_SELECTOR), function(h) {
  ...
  function getValuesForElement(e, t, n, r, o) {
    ...
    if (i) {
      ...
      if (s === "unset")
        return null;
      s.indexOf("javascript:") === 0 ? (s = s.slice(11), a = !0) : s.indexOf("js:") === 0 && (s = s.slice(3), a = !0), s.indexOf("{") !== 0 && (s = "{" + s + "}");
      let l;
      a ? l = maybeEval(e, function() {
  ...
  function issueAjaxRequest(e, t, n, r, o, i) {
    ...
    if (c == null || c == DUMMY_ELT)
      return triggerErrorEvent(n, "htmx:targetError", { target: getClosestAttributeValue(n, "hx-target") }), maybeCall(a), l;
    ...
    if (i === void 0 && triggerEvent(n, "htmx:confirm", { target: c, elt: n, path: t, verb: e, triggeringEvent: r, etc: o, issueRequest: function(H) {
      ...
      return maybeCall(s), l;
    let E = n, m = getClosestAttributeValue(n, "hx-sync"), w = null, C = !1;
    if (m) {
      ...
      if (m === "abort") {
        if (f.xhr)
          return maybeCall(s), l;
        C = !0;
      } else m === "replace" ? triggerEvent(E, "htmx:abort") : m.indexOf("queue") === 0 && (w = (m.split(" ")[1] || "last").trim());
    }
    if (f.xhr)
      if (f.abortable)
        ...
        if (w == null) {
          if (r) {
            ...
            g && g.triggerSpec && g.triggerSpec.queue && (w = g.triggerSpec.queue);
          }
          w == null && (w = "last");
        }
        return f.queuedRequests == null && (f.queuedRequests = []), w === "first" && f.queuedRequests.length === 0 ? f.queuedRequests.push(function() {
          issueAjaxRequest(e, t, n, r, o);
        }) : w === "all" ? f.queuedRequests.push(function() {
          issueAjaxRequest(e, t, n, r, o);
        }) : w === "last" && (f.queuedRequests = [], f.queuedRequests.push(function() {
          issueAjaxRequest(e, t, n, r, o);
        })), maybeCall(s), l;
      }
    const x = new XMLHttpRequest();
    f.xhr = x, f.abortable = C;
    const p = function() {
    ...
    if (!triggerEvent(n, "htmx:configRequest", S))
      return maybeCall(s), p(), l;
    if (t = S.path, e = S.verb, T = S.headers, R = formDataFromObject(S.parameters), D = S.errors, O = S.useUrlParams, D && D.length > 0)
      return triggerEvent(n, "htmx:validation:halted", S), maybeCall(s), p(), l;
    const k = t.split("#"), W = k[0], N = k[1];
    let A = t;
    if (O && (A = W, !R.keys().next().done && (A.indexOf("?") < 0 ? A += "?" : A += "&", A += urlEncode(R), N && (A += "#" + N))), !verifyPath(n, A, S))
      return triggerErrorEvent(n, "htmx:invalidPath", S), maybeCall(a), p(), l;
    if (x.open(e.toUpperCase(), A, !0), x.overrideMimeType("text/html"), x.withCredentials = S.withCredentials, x.timeout = S.timeout, !q.noHeaders) {
      for (const g in T)
    ...
    if (x.onload = function() {
      ...
      removeRequestIndicators(I, L), triggerErrorEvent(n, "htmx:afterRequest", v), triggerErrorEvent(n, "htmx:sendError", v), maybeCall(a), p();
    }, x.onabort = function() {
      removeRequestIndicators(I, L), triggerErrorEvent(n, "htmx:afterRequest", v), triggerErrorEvent(n, "htmx:sendAbort", v), maybeCall(a), p();
    }, x.ontimeout = function() {
      removeRequestIndicators(I, L), triggerErrorEvent(n, "htmx:afterRequest", v), triggerErrorEvent(n, "htmx:timeout", v), maybeCall(a), p();
  ...
  function determineHistoryUpdates(e, t) {
    ...
    const i = t.pathInfo.finalRequestPath, s = t.pathInfo.responsePath, a = getClosestAttributeValue(e, "hx-push-url"), l = getClosestAttributeValue(e, "hx-replace-url"), u = getInternalData(e).boosted;
    let h = null, c = null;
    return a ? (h = "push", c = a) : l ? (h = "replace", c = l) : u && (h = "push", c = s || i), c ? c === "false" ? {} : (c === "true" && (c = s || i), t.pathInfo.anchor && c.indexOf("#") === -1 && (c = c + "#" + t.pathInfo.anchor), {
      type: h,
      path: c
    }) : {};
  }
  ...
  function handleAjaxResponse(e, t) {
    ...
    if (hasHeader(n, /HX-Trigger:/i) && handleTriggerHeader(n, "HX-Trigger", e), hasHeader(n, /HX-Location:/i)) {
      ...
      let C = n.getResponseHeader("HX-Location");
      var s;
      C.indexOf("{") === 0 && (s = parseJSON(C), C = s.path, delete s.path), ajaxHelper("get", C, s).then(function() {
    ...
    const l = determineHistoryUpdates(e, t), u = resolveResponseHandling(n), h = u.swap;
    ...
    if (!(u.event && !triggerEvent(r, u.event, m)) && triggerEvent(r, "htmx:beforeSwap", m)) {
      if (r = m.target, E = m.serverResponse, c = m.isError, f = m.ignoreTitle, y = m.selectOverride, d = m.swapOverride, t.target = r, t.failed = c, t.successful = !c, m.shouldSwap) {
        ...
        var w = getSwapSpecification(e, d);
        w.hasOwnProperty("ignoreTitle") || (w.ignoreTitle = f), r.classList.add(htmx.config.swappingClass), i && (y = i), hasHeader(n, /HX-Reselect:/i) && (y = n.getResponseHeader("HX-Reselect"));
        const C = getClosestAttributeValue(e, "hx-select-oob"), x = getClosestAttributeValue(e, "hx-select");
        swap(r, E, w, {
